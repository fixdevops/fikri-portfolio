    ...
    <div className="bg-white min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      ...
      <section className="max-w-4xl mx-auto px-5 pt-4 text-gray-800">
        ...
        <div className="mt-5 text-center">
          ...
          <p className="text-sm text-gray-400 mt-2">Published on {formatDate(blog.published_at)} • {blog.reading_time || 1} min read</p>
        </div>
        {blog.thumbnail && (
          <div className="mt-5">
            <img src={blog.thumbnail} alt={blog.title} className="w-full rounded-lg shadow-lg" onError={(e) => e.target.style.display = 'none'} />
          </div>
        )}
        <article className="mt-10 text-[16px] leading-relaxed text-gray-800 text-justify prose max-w-none" dangerouslySetInnerHTML={{ __html: renderMarkdown(blog.content) }} />
      </section>
      <Footer />
    </div>
  );
